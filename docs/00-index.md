# Documentation Index

This project is documentation-first. Code starts only after tickets are generated, synced to GitHub Issues/Projects, and approved in a sprint.

## Current document map

### 00. Setup

- `docs/00-setup/agents-and-skills.md`
- `docs/00-setup/skills-inventory.md`

### 01. Research

- `docs/01-research/problem-statement.md`
- `docs/01-research/target-users.md`
- `docs/01-research/pain-points.md`
- `docs/01-research/competitor-analysis.md`
- `docs/01-research/opportunities.md`
- `docs/01-research/assumptions.md`
- `docs/01-research/risks.md`
- `docs/01-research/open-questions.md`
- `docs/01-research/research-summary.md`

### 02. Product

- `docs/02-product/product-vision.md`
- `docs/02-product/mvp-scope.md`
- `docs/02-product/out-of-scope.md`
- `docs/02-product/personas.md`
- `docs/02-product/user-journeys.md`
- `docs/02-product/success-metrics.md`

### 03. Requirements

- `docs/03-requirements/functional-requirements.md`
- `docs/03-requirements/non-functional-requirements.md`
- `docs/03-requirements/business-rules.md`

### 04. Use Cases

- `docs/04-use-cases/use-cases.md`

### 05. Architecture

- `docs/05-architecture/domain-model.md`

### 06. Diagrams

- `docs/06-diagrams/context-diagram.mmd`
- `docs/06-diagrams/container-diagram.mmd`
- `docs/06-diagrams/component-diagram.mmd`
- `docs/06-diagrams/sequence-diagrams.mmd`
- `docs/06-diagrams/erd.mmd`

### 07. ADRs

- `docs/07-adrs/adr-001-frontend-data-loading-and-contract-coordination.md`
- `docs/07-adrs/adr-002-frontend-architecture.md`
- `docs/07-adrs/adr-003-backend-architecture.md`
- `docs/07-adrs/adr-004-database-choice.md`
- `docs/07-adrs/adr-005-api-communication-pattern.md`
- `docs/07-adrs/adr-006-state-management-approach.md`
- `docs/07-adrs/adr-007-deployment-strategy.md`
- `docs/07-adrs/adr-008-monorepo-tooling.md`
- `docs/07-adrs/adr-009-testing-stack.md`
- `docs/07-adrs/adr-010-backend-persistence-implementation.md`
- `docs/07-adrs/adr-011-authentication-strategy.md`

### 08. Tech Decisions

- `docs/08-tech-decisions/final-stack.md`

### 09. Agile

- `docs/09-agile/definition-of-ready.md`
- `docs/09-agile/definition-of-done.md`
- `docs/09-agile/product-backlog.md`
- `docs/09-agile/sprint-plan.md`
- `docs/09-agile/sprint-backlog.md`

### 10. Monorepo

- `docs/10-monorepo/workspace-structure.md`
- `docs/10-monorepo/coding-standards.md`
- `docs/10-monorepo/package-boundaries.md`
- `docs/10-monorepo/dependency-policy.md`
- `docs/10-monorepo/build-pipeline.md`

### 11. Design System

- `docs/11-design-system/design-principles.md`
- `docs/11-design-system/design-tokens.md`
- `docs/11-design-system/typography-rules.md`
- `docs/11-design-system/spacing-rules.md`
- `docs/11-design-system/color-usage.md`
- `docs/11-design-system/component-inventory.md`
- `docs/11-design-system/component-states.md`
- `docs/11-design-system/accessibility-guidelines.md`
- `docs/11-design-system/ux-guidelines.md`
- `docs/11-design-system/storybook-documentation-strategy.md`

### 12. Frontend Architecture

- `docs/12-frontend-architecture/frontend-overview.md`
- `docs/12-frontend-architecture/ssr-strategy.md`
- `docs/12-frontend-architecture/routing.md`
- `docs/12-frontend-architecture/server-state.md`
- `docs/12-frontend-architecture/forms-and-validation.md`
- `docs/12-frontend-architecture/client-state.md`
- `docs/12-frontend-architecture/ui-composition.md`
- `docs/12-frontend-architecture/frontend-coding-standards.md`

### 13. UI Package + Storybook

- `docs/13-ui-package-storybook/ui-package-overview.md`
- `docs/13-ui-package-storybook/storybook-guidelines.md`
- `docs/13-ui-package-storybook/component-taxonomy.md`
- `docs/13-ui-package-storybook/component-testing.md`
- `docs/13-ui-package-storybook/release-versioning.md`

### 13. Testing

- `docs/13-testing/testing-strategy.md`
- `docs/13-testing/unit-testing.md`
- `docs/13-testing/integration-testing.md`
- `docs/13-testing/e2e-testing-playwright.md`
- `docs/13-testing/storybook-testing.md`
- `docs/13-testing/quality-gates.md`

### 14. GitHub Projects

- `docs/14-github-projects/github-projects-setup.md`
- `docs/14-github-projects/issue-template.md`
- `docs/14-github-projects/project-board-workflow.md`
- `docs/14-github-projects/labels-and-milestones.md`
- `docs/14-github-projects/sprint-to-github-projects.md`
- `docs/14-github-projects/ticket-lifecycle.md`
- `docs/14-github-projects/delivery-lanes.md`

### 15. Developer Experience

- `docs/15-developer-experience/dx-overview.md`
- `docs/15-developer-experience/monorepo-tooling.md`
- `docs/15-developer-experience/pnpm-workspace.md`
- `docs/15-developer-experience/nx-strategy.md`
- `docs/15-developer-experience/biome-strategy.md`
- `docs/15-developer-experience/lefthook-strategy.md`
- `docs/15-developer-experience/testing-dx.md`
- `docs/15-developer-experience/storybook-dx.md`
- `docs/15-developer-experience/github-actions-ci.md`
- `docs/15-developer-experience/github-projects-workflow.md`
- `docs/15-developer-experience/changesets-versioning.md`
- `docs/15-developer-experience/local-development.md`
- `docs/15-developer-experience/quality-gates.md`
- `docs/15-developer-experience/dx-decision-summary.md`

### 16. Backend Architecture

- `docs/16-backend-architecture/backend-overview.md`
- `docs/16-backend-architecture/hexagonal-architecture.md`
- `docs/16-backend-architecture/module-boundaries.md`
- `docs/16-backend-architecture/api-design.md`
- `docs/16-backend-architecture/database-strategy.md`
- `docs/16-backend-architecture/security-strategy.md`
- `docs/16-backend-architecture/backend-coding-standards.md`

## Operating rule

Use docs for scope and decisions, `docs/09-agile/` for approved planning, and GitHub Issues/Projects for execution state.
