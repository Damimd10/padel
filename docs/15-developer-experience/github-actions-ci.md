...
- run Biome checks
- run workspace module-boundary enforcement
...
4. run the workspace module-boundary gate
5. run affected unit/integration tests
6. run affected Storybook or UI validation when `packages/ui` changes
7. run affected E2E checks when required
8. run Changesets or release checks on release-oriented workflows

## CI philosophy

- CI should mirror local DX rather than invent a separate validation model
- use Nx affected and caching to keep CI efficient
- package-boundary enforcement should use the same command locally and in CI
